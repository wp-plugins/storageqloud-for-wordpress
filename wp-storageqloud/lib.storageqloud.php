...
*/   
class StorageQloudCrypt_HMAC {
var $_func;var $_ipad;var $_opad;var $_pack;
function StorageQloudCrypt_HMAC($key, $func = 'md5'){$this->setFunction($func);$this->setKey($key);}
...
*/
if (!class_exists('StorageQloudHTTPRequestCurl')) require_once (dirname(__FILE__).'/../lib/curl.php');
...
class StorageQloud {
	 ...
	 *
	 * - [str] $accessKeyId: Your GreenQloud Access Key Id
	 * - [str] $secretKey: Your GreenQloud Secret Access Key
	 * - [str] $serviceUrl: OPTIONAL: defaults: http://s.greenqloud.com/
	 *
	*/
	function StorageQloud($accessKeyId, $secretKey, $serviceUrl="http://s.greenqloud.com/") {
		global $wpdb;
		$this->serviceUrl=$serviceUrl;
		$this->accessKeyId=$accessKeyId;
		$this->secretKey=$secretKey;
		$this->req =& new StorageQloudHTTPRequestCurl($this->serviceUrl);
		$this->options = array();
		$this->options['cache_table'] = $wpdb->prefix . 'StorageQloud_cache';
	...
	*/   
	function setObjectACL($bucket, $key, $acl){
		$serviceUrl = 'http://'.$bucket.'.s.greenqloud.com/';
    ...
    */
	...
	function send($bucket, $resource, $args='', $method='GET', $headers=false) {
		if ($bucket != '') {
			...
			$serviceUrl = 'http://s.greenqloud.com/';
		}
	...
	function constructSig($str) {
		$hasher =& new StorageQloudCrypt_HMAC($this->secretKey, "sha1");
